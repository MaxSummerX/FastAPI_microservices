...
async def process_category_check(message: AbstractIncomingMessage, default_exchange: AbstractExchange) -> None:
    ...
    async with message.process():
        ...
        try:
            ...
            print(f" [!] Ошибка: не удалось распознать ID категории из сообщения: {message.body.decode()}")
...
async def run_consumer() -> None:
    ...
    try:
        ...
        async with connection:
            ...
            await queue.consume(
                lambda message: process_category_check(message=message, default_exchange=default_exchange)
            )
        ...
        if connection and not connection.is_closed:
            ...
            print("Соединение RabbitMQ закрыто.")
